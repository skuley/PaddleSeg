...
class BasePredictor(object):
    def __init__(self,
        ...
        state_dict = torch.load('disnet.ckpt', map_location=self.device)['state_dict']
        new_state_dict = {}
        for key, value in state_dict.items():
            new_key = key.replace('model.', '')
            new_state_dict[new_key] = value
        self.disnet.load_state_dict(new_state_dict, strict=False)
        self.disnet = self.disnet.to(self.device)
        self.disnet.eval()
        print('disnet loaded')
    ...
    def get_disnet(self, shape):
        ...
        with torch.no_grad():
            outputs, _ = self.disnet(tn_img.to(self.device).unsqueeze(0))
